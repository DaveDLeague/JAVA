...
final String[] WILDCARD_IMPORTS = {"java.util.*", "java.awt.*", "java.util.*", "java.util.regex.*", "java.io.*", "javax.swing.*", "java.text.*", "java.awt.event.*"};
...
enum GameStates {
  TITLE_STATE, LOAD_GAME_STATE, MENU_SCREEN_STATE, WORLD_MAP_STATE, HOW_TO_PLAY_STATE, CHARACTER_SELECT_STATE, BETWEEN_WORLDS_QUIZ_STATE, QUIZ_RUNNING_STATE
}
...
String[] generateWordArray(int words){
  String[] w = new String[words];
  int sa = 0;
  while(sa < words){
    String rw = WORD_LIST[(int)random(WORD_LIST.length)];
    boolean found = false;
    for(int i = 0; i < sa; i++){
      if(rw.equals(w[i])){
        found = true;
...
String generateRandomHexString(int minDigits, int maxDigits) {
  String num = "";
  char[] hexChars = {'a', 'b', 'c', 'd', 'e', 'f', 'A', 'B', 'C', 'D', 'E', 'F'};
  int digits = (int)random(minDigits, maxDigits);
  for (int i = 0; i < digits; i++) {
    if(random(2) < 1){
    num += (int)random(10);
    }else{
      num += hexChars[(int)random(hexChars.length)];
    }
  }
  return num;
}

String generateRandomHexString(int maxDigits) {
  return generateRandomHexString(1, maxDigits);
}
...
boolean renderDialogChoice(String text) {
  ...
  textSize(h);
  float tw = textWidth(text);
  boolean ret = renderDialogChoice(text, resolutionWidth / 2 - tw / 2, dialogChoiceYPos, defaultDialogBoxOffColor, defaultDialogTextOffColor);
  dialogChoiceYPos += dialogChoiseYIncrement;
  return ret;
}

boolean renderDialogChoice(String text, float x, float y){
   return  renderDialogChoice(text, x, y, defaultDialogBoxOffColor, defaultDialogTextOffColor);
}

boolean renderDialogChoice(String text, float x, float y, color bc, color tc) {
  float h = textBoxTextSize;
  textSize(h);
  float tw = textWidth(text);
  boolean ret = false;
  ...
  if (scaledMouseX > x - 10 && scaledMouseX < x + tw + 10 && 
    scaledMouseY > y - 10 && scaledMouseY < y + h + 25) {
